...
class Gerador(Agent):
    ...
    class printFunction(OneShotBehaviour):
        async def run(self):
            termos = list(reversed([f"(({coef})*(x^{i}))" for i, coef in enumerate(reversed(self.agent.coeficientes))]))
            print(termos)
            funcao = " + ".join(termos)
    ...
    class setDegree(CyclicBehaviour):
        async def run(self):
            ...
            if msg_received:
                ...
                self.agent.grau = 1 #random.randint(1,3)
    ...
    class FunctionSolver(CyclicBehaviour):
        async def run(self):
            ...
            if msg_received:
                ...
                x_variando = list(reversed([f"(({coef})*(x^{i}))" for i, coef in enumerate(reversed(self.agent.coeficientes))]))
                print(f"Gerador: f({x}) = {' + '.join(x_variando)}")
